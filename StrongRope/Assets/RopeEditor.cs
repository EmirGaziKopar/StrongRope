...
public class RopeEditor : Editor
{
    ...
    private void DeleteHandleFunc(int controlID, Vector3 position, Quaternion rotation, float size, EventType eventType)
    {
        throw new System.NotImplementedException();
    }

    private void HandleFunc(int controlID, Vector3 position, Quaternion rotation, float size, EventType eventType)
    {
    ...
    private void DrawHinges(Rope rope)
    {
        ...
        if (rope.WithPhysics && rope.HangLastSegment)
        {
            ...
            if (GetConnectedObject(hingePositionInWorldSpace, lastSegment.GetComponent<Rigidbody2D>()))
                ...
                GUI.color = Color.blue;
            GUI.color = Color.white;
            if ((Vector2)Handles.FreeMoveHandle(hingePositionInWorldSpace, Quaternion.identity, handleSize * 0.05f, Vector3.one, SolidHandleFunc) != hingePositionInWorldSpace)
            {
                rope.LastSegmentConnectionAnchor = rope.transform.InverseTransformPoint((Vector2)Handles.FreeMoveHandle(hingePositionInWorldSpace, Quaternion.identity, handleSize * 0.05f, Vector3.one, SolidHandleFunc));
                UpdateEndsJoints(rope);
            }
        }
    }

    private void SolidHandleFunc(int controlID, Vector3 position, Quaternion rotation, float size, EventType eventType)
    {
